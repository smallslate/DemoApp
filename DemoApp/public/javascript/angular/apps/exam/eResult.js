...
examResultApp.controller('examResultCtrl', ['$scope','examResultService',function($scope,examResultService) {
	$scope.evaluateQuestion = function(answer,questionId) {
		$scope.examObj = examResultService.query({action:'evalQuestion',answer:answer,questionId:questionId,sessionCode:$scope.examObj.examSession.sessionCode},function(data) {
			renderResults();
		});
	};
